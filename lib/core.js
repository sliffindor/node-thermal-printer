...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    switch (initConfig.type) {
      ...
      case this.types.BROTHER:
        ...
        break;
      case this.types.SAM4S:
        const Sam4s = require('./types/sam4s');
        this.printer = new Sam4s();
        break;
      default:
  ...
  // ----------------------------------------------------- LEFT RIGHT -----------------------------------------------------
  leftRight(left, right, currentSize = 1) {
    this.append(left.toString());
    const width = this.config.width/currentSize - getTextLength(left.toString()) - getTextLength(right.toString());
    ...
    if( width >= 0 ) {
      ...
      this.newLine();
      for (let i = 0; i < this.config.width/currentSize - getTextLength(right.toString()); i++) {
        this.append(Buffer.from(' '));
      }
      this.append(right.toString());
    }
